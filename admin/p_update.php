...
if (isset($_POST['save'])) {
    ...
    $sql = mysqli_query($conn, "UPDATE products SET p_name='$name', p_image='$image_name',p_price='$price',p_disc='$about',category='$category' WHERE id=$id;");
    if ($sql == TRUE) {
        header('location:products.php');
        $_SESSION['deleted'] = "<p class='text-success'>Product Updated Successfully!</p>";
    } else {
        header('location:products.php');
        $_SESSION['e-deleted'] = "<p class='text-danger'>Error Updating Product!</p>";
    }
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <section id="p-info">
        <h1 style="text-align: center;margin-top:3%">Update Product</h1>
        <!-- customer info -->
        <div class="infor">
            <?php
                ...
                <tbody class="text-center">
                    <tr>
                        <form method="post" class="gg" enctype="multipart/form-data">
                            ...
                            <td>
                                <input class="text-center form-control" type="text" name="price" value="<?php echo $price ?>">

                            </td>
                            <td>
                                <textarea class="form-control" type="text" name="about"><?php echo $about ?></textarea>
                            ...
                            <td>
                                ...
                                <option class="selected text-center" value="-">-</option><?php
                                while ($row = mysqli_fetch_assoc($sql)) {
                                    $cate=$row['c_name'];
                                ?>
