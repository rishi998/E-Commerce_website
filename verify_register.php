...
if (isset($_POST["check"])) {
    ...
    $ress = mysqli_query($conn, "SELECT * FROM otp_expiry WHERE otp=$otp AND is_expired!=1 AND NOW() <= DATE_ADD(create_at, INTERVAL 2 MINUTE)");
    $count  = mysqli_num_rows($ress);
    if ($count > 0) {
        $result = mysqli_query($conn, "UPDATE otp_expiry SET is_expired = 1 WHERE otp = $otp");
        $mail = $_SESSION['b-email'];
        $fullname = $_SESSION['b-fname'];
        $username = $_SESSION['b-userr'];
        $passsword = $_SESSION['b-pask'];
